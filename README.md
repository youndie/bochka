...
2. **Live clients, from the first milestone rather than after it** — `aws-cli`, `boto3`, `rclone`,
   ...
   object equal to one of them rather than a mixture of two. Those four are containers looking at
   the server from outside the process, which cannot be the client of the **embedded** mode: on the
   JVM that client is a library in the same JVM. So `bochka-embedded` carries its own foreign
   client — `io.minio:minio`, eleven operations and a multipart upload — and it runs in the gate
   rather than in a script that needs docker.
...
**215 of the 240 remaining failures** are things this store says in
